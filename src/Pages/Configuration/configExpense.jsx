...
const ConfigExpense = () => {
  ...
  const {
    ...
    total_count = 0,
    loading = false,
  } = useSelector((state) => state.auth.categories || {});
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        await dispatch(fetchExpenseCategories({ page: 1, limit: pageSize }));
      } finally {
  ...
  const loadCategories = (page, size) => {
    dispatch(
      fetchExpenseCategories({
        page,
        limit: size,
  ...
  const handlePageChange = (newPage) => {
    ...
    loadCategories(newPage, pageSize);
  };

  const handlePageSizeChange = (e) => {
    const newSize = parseInt(e.target.value);
    setPageSize(newSize);
    loadCategories(1, newSize); // Reset to first page when changing page size
  };
  ...
  const validatePartyName = (name) => {
    ...
    if (!name) {
      error =
        "Expense name cannot be blank, please enter a valid expense name.";
    }
    return error;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = isEditMode
        ...
        : await dispatch(createExpenseCategory(payload));
      ...
      if (result?.errorcode === 0) {
        ...
        resetForm();
        loadCategories(currentPage, pageSize); // Reload current page
    ...
    if (confirm.isConfirmed) {
      try {
        ...
        if (result?.errorcode === 0) {
          ...
          dispatch(fetchExpenseCategories()); // Refresh list
        } else {
  ...
  const handleToggleStatus = async (cat) => {
    ...
    try {
      const result = await dispatch(toggleCategoryStatus(cat.id, newStatus));
        ...
        <div className="col-xl-12">
          <div className="card custom-card">
            <div className="card-header justify-content-between">
              <div className="card-title">Configuration Expense</div>
              <div>
                {permissions.add && (
                  <button
            ...
            <div className="card-footer">
              <div className="d-flex flex-wrap align-items-center justify-content-between w-100">
                <div>
                  Showing {categories.length} of {total_count} entries
                </div>
