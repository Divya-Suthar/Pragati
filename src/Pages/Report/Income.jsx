...
import { fetchIncomeExpenseReport } from "../../redux/actions/authActions";
...
const Income = () => {
  ...
  const [sortColumn, setSortColumn] = useState(""); // New state for sort column
  ...
  const { incomeExpenseData, loading, error, total_count } = useSelector(
    (state) => state.auth
  );
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setIsPageLoad(true);
      try {
        await dispatch(fetchIncomeExpenseReport(params));
      } finally {
        setIsPageLoad(false);
      }
    };
  ...
  const sortData = (data, column, direction) => {
    ...
    return [...data].sort((a, b) => {
      ...
      if (column === "created_at") {
        ...
        // For Income and Expense, use amount based on transaction_type_name
        valueA =
          a.transaction_type_name === "Income" ? parseFloat(a.amount) || 0 : 0;
        valueB =
          b.transaction_type_name === "Income" ? parseFloat(b.amount) || 0 : 0;
        if (column === "expense_money_out") {
          valueA =
            a.transaction_type_name === "Expense"
              ? parseFloat(a.amount) || 0
              : 0;
          valueB =
            b.transaction_type_name === "Expense"
              ? parseFloat(b.amount) || 0
              : 0;
        }
      } else if (column === "party_or_user_name") {
  ...
  const sortedData = useMemo(() => {
    if (!incomeExpenseData || incomeExpenseData.length === 0) return [];
    return sortData(incomeExpenseData, sortColumn, sortDirection);
  }, [incomeExpenseData, sortColumn, sortDirection]);
  ...
  const handleDownloadExcel = async () => {
    try {
      ...
      const response = await fetch(
        `${base_url}/reports/income-vs-expense-report-excel-download?${params}`,
        {
          method: "GET",
          headers: {
            Authorization: ` ${token}`,
      ...
      if (response.ok) {
        ...
        link.download = "IncomeVsExpense_List.xlsx";
        link.click();
      } else {
        ...
        <div className="col-xl-12">
          <div className="card custom-card">
            <div className="card-header justify-content-between">
              <div className="card-title">Income vs. Expense Report</div>
            </div>
                ...
                <table className="table text-nowrap table-bordered">
                  <thead style={{ backgroundColor: "rgb(209, 212, 221)" }}>
                    <tr>
                      <th
                        scope="col"
                        onClick={() => handleSort("party_or_user_name")}
                        style={{ cursor: "pointer" }}
                      >
                        Name {renderSortIcon("party_or_user_name")}
                      </th>
                      <th
                        scope="col"
                      ...
                      <th
                        ...
                        style={{ cursor: "pointer" }}
                      >
                        Income Money In {renderSortIcon("amount")}
                      </th>
                      <th
                        scope="col"
                        onClick={() => handleSort("expense_money_out")}
                        style={{ cursor: "pointer" }}
                      >
                        Expense Money Out {renderSortIcon("expense_money_out")}
                      </th>
                    ...
                    {Array.isArray(filteredData) && filteredData.length > 0 ? (
                      filteredData.map((item) => (
                        <tr key={item.tran_id || item._id}>
                          <td>{item.party_or_user_name || "-"}</td>
                          <td>{formatDate(item.created_at)}</td>
                          <td>{item.expense_category_name || "-"}</td>
                          <td>
                            {item.transaction_type_name === "Income"
                              ? formatINR(item.amount) || "-"
                              : "-"}
                          </td>
                          <td>
                            {item.transaction_type_name === "Expense"
                              ? formatINR(item.amount) || "-"
                              : "-"}
                          </td>
                          <td>
                            {formatINR(item.cumulative_balance) || "0.00"}
                          </td>
                        </tr>
                      ))
                    ) : (
