...
import { fetchBrokerageReport } from "../../redux/actions/authActions";
...
const Brokerage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setIsPageLoad(true);
      try {
        await dispatch(fetchBrokerageReport(params));
      } finally {
        setIsPageLoad(false);
      }
    };
  ...
  const handleDownloadExcel = async () => {
    try {
      ...
      const response = await fetch(
        `${base_url}/reports/brokerage-report-excel-download?${params}`,
        {
          method: "GET",
          headers: {
            Authorization: ` ${token}`,
      ...
      if (response.ok) {
        ...
        link.download = "Brokerage_List.xlsx";
        link.click();
      } else {
        ...
        <div className="col-xl-12">
          <div className="card custom-card">
            <div className="card-header justify-content-between">
              <div className="card-title">Brokerage and Interest Report</div>
            </div>
                    ...
                    {Array.isArray(filteredData) && filteredData.length > 0 ? (
                      filteredData.map((item) => (
                        <tr key={item.tran_id || item._id}>
                          ...
                          <td>{item.brokerage ? `${item.brokerage}%` : "-"}</td>
                          <td>{formatINR(item.brokerage_amount)}</td>
                          <td>{item.interest ? `${item.interest}%` : "-"}</td>
                          <td>{formatINR(item.interest_amount)}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
